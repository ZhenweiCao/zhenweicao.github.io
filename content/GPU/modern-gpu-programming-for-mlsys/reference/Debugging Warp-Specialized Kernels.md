...
GEMM Steps 7-9 in [[GPU/modern-gpu-programming-for-mlsys/part-3-gemm/14 Scaling GEMM with Warp Specialization and Clusters|Scaling GEMM with Warp Specialization and Clusters]] overlap TMA load, `tcgen05` MMA, and TMEM/SMEM writeback. The same debugging method applies to Flash Attention handoffs: identify the roles, identify the storage each role owns, then verify the generated CUDA against that model.
...
|---|---|---|
| Unknown TIRx API or attribute error | Installed wheel does not match the tutorial code | Print `tvm.__file__` and `tvm.__version__`; compare the API name with [[GPU/modern-gpu-programming-for-mlsys/reference/TIRx Language Reference|TIRx Language Reference]]. |
...
[[GPU/modern-gpu-programming-for-mlsys/reference/Reference|Previous]] · [[GPU/modern-gpu-programming-for-mlsys/README|Book index]] · [[GPU/modern-gpu-programming-for-mlsys/reference/TIRx Language Reference|Next]]
