...
aliases:
  - "Buffers and memory"
source: https://github.com/mlc-ai/modern-gpu-programming-for-mlsys/blob/8950d661e8499008546e3520c667c1cacec9af21/tirx_guide/language_reference/cuda/buffers.rst
...
A buffer's `data` pointer is an immutable `Var` (`alloc_buffer` defines it; `decl_buffer` takes one). To back a buffer with a pointer *expression*, bind it first — see [[GPU/modern-gpu-programming-for-mlsys/reference/tirx-language/Data Types and Expressions|Data types and expressions]].
...
| `layout` | physical mapping ([[GPU/modern-gpu-programming-for-mlsys/part-2-tirx/11 TIRx Layout API\|TileLayout]]); `"default"` = dense row-major |
...
```
(Two separate `alloc_buffer(scope="shared.dyn")` calls are an error — *only one dynamic shared memory allocation is allowed*.) So static shared memory is sized at compile time (`__shared__ T x[N];`); dynamic shared memory is this one launch-sized arena with views decl'd at offsets inside it.
...
```
**Vectorized access — `vload` / `vstore`.** Move several elements as one wide transfer (see also [[GPU/modern-gpu-programming-for-mlsys/reference/tirx-language/Data Types and Expressions|Data types and expressions]]):
...
[[GPU/modern-gpu-programming-for-mlsys/reference/tirx-language/Data Types and Expressions|Previous]] · [[GPU/modern-gpu-programming-for-mlsys/README|Book index]] · [[GPU/modern-gpu-programming-for-mlsys/reference/tirx-language/Control Flow|Next]]
