...
The pipelined GEMM from the previous chapter ([[GPU/modern-gpu-programming-for-mlsys/part-3-gemm/13 Pipelining GEMM with TMA|Pipelining GEMM with TMA]]) is fast, but it still asks one warpgroup to do everything: issue the load, run the MMA, then write the result back. Even with a software pipeline, that one team of threads becomes the place where all three engines meet.
...
We pursue that idea in three steps of widening cooperation. Step 7 ([[GPU/modern-gpu-programming-for-mlsys/part-3-gemm/14 Scaling GEMM with Warp Specialization and Clusters#Step 7: Warp Specialization + Pipeline|Scaling GEMM with Warp Specialization and Clusters]]) specializes warps into producer, consumer, and writeback roles. Step 8 ([[GPU/modern-gpu-programming-for-mlsys/part-3-gemm/14 Scaling GEMM with Warp Specialization and Clusters#Step 8: 2-CTA Cluster|Scaling GEMM with Warp Specialization and Clusters]]) joins two CTAs into a cluster that shares operands across their shared memory. Step 9 ([[GPU/modern-gpu-programming-for-mlsys/part-3-gemm/14 Scaling GEMM with Warp Specialization and Clusters#Step 9: Multi-Consumer Warp Specialization|Scaling GEMM with Warp Specialization and Clusters]]) adds a second MMA consumer so one staged tile feeds twice the math.
...
![[GPU/modern-gpu-programming-for-mlsys/assets/images/warp_specialization_timeline.png]]

*Warp Specialization Timeline*
...
**Interactive: A 2-CTA cluster: cooperative MMA via cross-CTA SMEM read.** [[GPU/modern-gpu-programming-for-mlsys/assets/interactive/cta_cluster.html|Open as a full page]]
...
Every time in this table, the 70 ms Step 1 baseline included, is measured at the same M=N=K=4096 size, which is what makes the speedup chain comparable end to end. It is worth being precise about what that 70 ms actually is, since it is easy to misread. It is *not* the single-tile Step-1 kernel from [[GPU/modern-gpu-programming-for-mlsys/part-3-gemm/12 Building a Tiled GEMM|Building a Tiled GEMM]] run at 4096³; that kernel only ever computes one 128×128 tile and only runs at small sizes. The 70 ms is instead a naive full-size baseline that takes the same sequential, single-tile approach and scales it up to the full 4096³ problem. Steps 1–3 are introduced in [[GPU/modern-gpu-programming-for-mlsys/part-3-gemm/12 Building a Tiled GEMM|Building a Tiled GEMM]] at small sizes (128×128 and 256³) to keep those first walkthroughs simple; the Step 1 and Step 3 rows here are their full-size benchmark counterparts. The remaining dashes (Steps 2, 5, 6) mark steps shown for structure but not timed on their own.
...
[[GPU/modern-gpu-programming-for-mlsys/part-3-gemm/13 Pipelining GEMM with TMA|Previous]] · [[GPU/modern-gpu-programming-for-mlsys/README|Book index]] · [[GPU/modern-gpu-programming-for-mlsys/part-4-flash-attention/15 Flash Attention 4|Next]]
