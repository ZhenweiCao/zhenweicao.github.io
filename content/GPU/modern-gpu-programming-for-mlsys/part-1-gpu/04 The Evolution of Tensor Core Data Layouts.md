...
![[GPU/modern-gpu-programming-for-mlsys/assets/images/ldstmatrix.svg]]

*ldmatrix loads an 8x8 shared memory tile into a warp register fragment; the reverse stmatrix path is available on Hopper sm_90 and later architectures*
...
![[GPU/modern-gpu-programming-for-mlsys/assets/images/swizzle_conflict.svg]]

*In a plain row-major tile, row writes spread across banks while a column read collides on one bank; XOR swizzling scatters the column read without giving up coalesced row writes*
...
![[GPU/modern-gpu-programming-for-mlsys/assets/images/wgmma_descriptor_kmajor.svg]]

*For a K-major 128-byte swizzle, the matrix descriptor uses the start address and `sdo` to locate an eight-row group; the fixed atom layout and XOR swizzle then determine the byte position*
...
![[GPU/modern-gpu-programming-for-mlsys/assets/images/sf_scale_vec.svg]]

*`scale_vec` packing: 1X repeats one scale across four bytes, 2X repeats a pair of scales, and 4X stores four distinct K-block scales*
...
[[GPU/modern-gpu-programming-for-mlsys/part-1-gpu/03 Data Layout and Its Notation|Previous]] · [[GPU/modern-gpu-programming-for-mlsys/README|Book index]] · [[GPU/modern-gpu-programming-for-mlsys/part-1-gpu/05 Async Data Movement - TMA|Next]]
