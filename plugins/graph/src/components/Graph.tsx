...
export default ((userOpts?: Partial<GraphOptions>) => {
  const Graph: QuartzComponent = ({ displayClass, cfg }: QuartzComponentProps) => {
    ...
    const globalGraphLabel = cfg.locale?.startsWith("zh")
      ? "打开完整关系图谱"
      : "Open global graph";
    const collapseGraphLabel = cfg.locale?.startsWith("zh") ? "收起图谱" : "Collapse graph";
    const expandGraphLabel = cfg.locale?.startsWith("zh") ? "展开图谱" : "Expand graph";
      ...
      <div class={classNames(displayClass, "graph")}>
        <div class="graph-header">
          ...
          <button
            ...
            type="button"
            aria-expanded="true"
            aria-label={collapseGraphLabel}
            title={collapseGraphLabel}
            data-collapse-label={collapseGraphLabel}
            data-expand-label={expandGraphLabel}
          >
            <span class="graph-toggle-label">{collapseGraphLabel}</span>
            <span class="graph-toggle-chevron" aria-hidden="true"></span>
          </button>
        </div>
        <div class="graph-outer">
          <div class="graph-container" data-cfg={JSON.stringify(localGraph)}></div>
          <button class="global-graph-icon" aria-label={globalGraphLabel} title={globalGraphLabel}>
            <svg
              version="1.1"
