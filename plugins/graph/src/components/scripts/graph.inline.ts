...
import { selectGraphData } from "../graph-data";
import { graphPanelIsCollapsed, storeGraphPanelState } from "../graph-panel";
import { createRenderEpoch } from "../graph-lifecycle";

(function () {
  var graphStorage = getBrowserStorage();
  var graphRuntimePromise = null;
  var graphRuntimeInitialized = false;
  ...
  function isGraphPanelCollapsed() {
    return graphPanelIsCollapsed(graphStorage);
  }
  ...
  function applyGraphPanelState(collapsed) {
    var graphs = document.querySelectorAll(".graph");
    for (var i = 0; i < graphs.length; i++) {
      graphs[i].classList.toggle("is-collapsed", collapsed);
    }

    var buttons = document.querySelectorAll(".graph-toggle");
    for (var i = 0; i < buttons.length; i++) {
      var button = buttons[i];
      var label = collapsed
        ? button.dataset["expandLabel"] || "Expand graph"
        : button.dataset["collapseLabel"] || "Collapse graph";
      button.setAttribute("aria-expanded", String(!collapsed));
      button.setAttribute("aria-label", label);
      button.setAttribute("title", label);

      var labelElement = button.querySelector(".graph-toggle-label");
      if (labelElement) {
  ...
  function emitGraphPanelChange(collapsed) {
    document.dispatchEvent(
      new CustomEvent("graphpanelchange", {
  ...
  function handleGraphPanelClick(event) {
    ...
    var button = target?.closest?.(".graph-toggle");
    if (!button) return;

    var collapsed = !isGraphPanelCollapsed();
    storeGraphPanelState(graphStorage, collapsed);
    applyGraphPanelState(collapsed);
    emitGraphPanelChange(collapsed);

    if (!collapsed && !isReaderModeActive()) {
      ensureGraphRuntime();
    }
  }
  ...
  function loadScript(src) {
    ...
    if (existing) {
      if (existing.dataset["graphLoaded"] === "true") {
    ...
    return new Promise(function (resolve, reject) {
      ...
      script.onload = function () {
        script.dataset["graphLoaded"] = "true";
        resolve();
      };
  ...
  function showGraphLoadError(error) {
    console.error("[Graph] Failed to load libraries:", error);
    var containers = document.querySelectorAll(".graph:not(.is-collapsed) .graph-container");
    for (var i = 0; i < containers.length; i++) {
      containers[i].textContent = "关系图谱加载失败，请检查网络后重试。";
      containers[i].classList.add("graph-load-error");
    }
  }

  function ensureGraphRuntime() {
    if (graphRuntimeInitialized || isGraphPanelCollapsed() || isReaderModeActive()) {
      return Promise.resolve();
    }
    if (graphRuntimePromise) return graphRuntimePromise;

    graphRuntimePromise = Promise.all([
      window.d3
        ? Promise.resolve()
      ...
      .then(function () {
        if (!window.d3 || !window.PIXI) {
          throw new Error("Graph libraries did not expose their browser globals");
        }
        if (!graphRuntimeInitialized) {
      ...
      .catch(function (error) {
        ...
        showGraphLoadError(error);
      });

    return graphRuntimePromise;
  }

  document.addEventListener("click", handleGraphPanelClick);
  document.addEventListener("nav", syncGraphPanel);
  document.addEventListener("render", syncGraphPanel);
  document.addEventListener("readermodechange", function (event) {
    if (event.detail?.mode === "off" && !isGraphPanelCollapsed()) {
      ensureGraphRuntime();
    }
  });
  ...
  function initGraph() {
    ...
    if (!d3 || !PIXI) {
      showGraphLoadError(new Error("Graph libraries not loaded"));
      return;
    }
    ...
    function getVisited() {
      if (!graphStorage) return new Set();

      try {
        var storedValue = JSON.parse(graphStorage.getItem(localStorageKey) || "[]");
        return new Set(Array.isArray(storedValue) ? storedValue : []);
      } catch (_) {
        try {
          graphStorage.setItem(localStorageKey, "[]");
        } catch (_) {
          // Ignore storage failures; visited state is only a visual preference.
        }
        return new Set();
      }
    }
    ...
    function addToVisited(slug) {
      ...
      visited.add(slug);
      try {
        graphStorage?.setItem(localStorageKey, JSON.stringify(Array.from(visited)));
      } catch (_) {
        // Ignore storage failures; graph rendering should continue.
      }
    }
    ...
    async function renderGraph(graph, fullSlug, isStale) {
      ...
      graph.classList.remove("graph-load-error");
      ...
      var onlyDirectLinks = config.onlyDirectLinks === true;
      var maxNodes = Number.isFinite(config.maxNodes) ? config.maxNodes : 0;
      ...
      data.forEach(function (_, id) {
        if (showTags || !id.startsWith("tags/")) {
          validLinks.add(id);
        }
      });
      ...
      var selection = selectGraphData({
        currentSlug: slug,
        links: links,
        validNodeIds: validLinks,
        tagNodeIds: allTags,
        depth: depth,
        onlyDirectLinks: onlyDirectLinks,
        maxNodes: maxNodes,
      });
      var neighbourhood = selection.nodes;
      ...
      var graphLinks = [];
      for (var i = 0; i < selection.links.length; i++) {
        var link = selection.links[i];
      ...
      if (renderIsStale()) {
        try {
          ...
          // PixiJS may throw when initialization was interrupted.
        }
        return function () {};
      }
    ...
    var localCleanups = [];
    var globalCleanups = [];
    var localRenderEpoch = createRenderEpoch();
    var globalRenderEpoch = createRenderEpoch();

    function cleanupLocal() {
      localRenderEpoch.invalidate();
    ...
    function showGlobalGraph() {
      if (isGraphPanelCollapsed() || isReaderModeActive()) return;

      cleanupGlobal();
      var renderEpoch = globalRenderEpoch.begin();
      ...
      for (var i = 0; i < globalContainers.length; i++) {
        ...
        if (graphContainer) {
          (function (modal, gc, epoch) {
            renderGraph(gc, currentSlug, function () {
              return (
                !globalRenderEpoch.isCurrent(epoch) ||
          ...
          })(container, graphContainer, renderEpoch);
        }
      }
    }
    ...
    function renderLocal() {
      ...
      var renderEpoch = localRenderEpoch.begin();
      var slug = getSlugFromUrl();
      addToVisited(slug);

      var localContainers = document.querySelectorAll(".graph:not(.is-collapsed) .graph-container");
      for (var i = 0; i < localContainers.length; i++) {
        (function (container, epoch) {
          renderGraph(container, slug, function () {
            return (
              !localRenderEpoch.isCurrent(epoch) ||
              isGraphPanelCollapsed() ||
              isReaderModeActive() ||
    ...
    function handleNav(e) {
      ...
      if (anyGlobalGraphActive()) {
        if (isGraphPanelCollapsed() || isReaderModeActive()) {
          hideGlobalGraph();
        } else {
          showGlobalGraph();
        }
    ...
    function handleGraphPanelChange(event) {
      if (event.detail?.collapsed) {
        cleanupLocal();
        hideGlobalGraph();
      } else if (!isReaderModeActive()) {
        renderLocal();
      }
    }

    function handleReaderModeChange(event) {
      if (event.detail?.mode === "on") {
        cleanupLocal();
        hideGlobalGraph();
      } else if (!isGraphPanelCollapsed()) {
        renderLocal();
      }
    }
    ...
    document.addEventListener("graphpanelchange", handleGraphPanelChange);
